...
from scipy.io import savemat #para guardar un archivo .mat
...
# =========================
# 2) FRASES
# Deben dar exactamente 8 tokens:
# [CLS] + 5 tokens simples + . + [SEP]
# =========================
positive_sentences = [
...
# =========================
if len(all_sentences) != 40: #comprueba explícitamente que haya 40 frases.
    raise ValueError(f"Debe haber 40 frases en total y hay {len(all_sentences)}.")

# =========================
# 4) FUNCIÓN: COMPROBAR TOKENIZACIÓN
# =========================
def check_sentence_tokens(sentences, tokenizer):
    ...
    for s in sentences:
        ...
        valid_sentences.append((s, tokens))

    return valid_sentences #permite conservar qué tokens produce cada frase.

print("Verificando frases positivas...\n")
valid_positive = check_sentence_tokens(positive_sentences, tokenizer)

print("\nVerificando frases negativas...\n")
valid_negative = check_sentence_tokens(negative_sentences, tokenizer)

print("\nTodas las frases producen exactamente 8 tokens.\n")

# =========================
# 5) FUNCIÓN: OBTENER VECTOR DE 9216
# Orden:
# capa 0 -> head 0 -> filas completas
# ...
# capa 11 -> head 11
# =========================
def get_attention_vector(attentions):
    ...
    for layer in range(len(attentions)):  # 12 capas
        ...
        for head in range(layer_att.shape[0]):  # 12 heads
            ...
            vector.extend(head_matrix.flatten())  # fila por fila

    return np.array(vector, dtype=np.float32)

# =========================
# 6) FUNCIÓN: PROCESAR UNA FRASE
# =========================
...
# =========================
# 7) CONSTRUIR MATRIZ FINAL 40 x 9216
# =========================
matrix_40x9216 = []
tokens_per_sentence = []
prob_negative = []
...
for i, sentence in enumerate(all_sentences):
    ...
    print(f"Frase {i+1:02d}/40 procesada")
...
# =========================
print("\nShape final de la matriz:", matrix_40x9216.shape)
# Debe ser (40, 9216)

# =========================
# 9) GUARDAR CSV PRINCIPAL
# =========================
...
print("Archivo guardado: attention_matrix_40x9216.csv")

# =========================
# 10) GUARDAR CSV DE TOKENS
# =========================
...
# =========================
# 11) GUARDAR EN FORMATO MATLAB .MAT
# =========================
mat_data = { #este codigo genera la matriz de atencion global final en formato matlab para poder hacerle el análisis multivariante posteriormente desde matlab
    "X": matrix_40x9216,                       # matriz principal 40x9216
    "sentences": np.array(all_sentences, dtype=object),
    "true_label": np.array(labels, dtype=object),
    "predicted_label": np.array(predicted_label, dtype=object),
    "prob_negative": np.array(prob_negative, dtype=np.float32),
    "prob_positive": np.array(prob_positive, dtype=np.float32)
}

savemat("attention_data_40x9216.mat", mat_data)
print("Archivo guardado: attention_data_40x9216.mat")

# =========================
# 12) INFO FINAL
# =========================
print("\nProceso completado correctamente.")
print("Se han generado estos archivos:")
print(" - attention_matrix_40x9216.csv")
print(" - attention_tokens_40_sentences.csv")
print(" - attention_data_40x9216.mat")