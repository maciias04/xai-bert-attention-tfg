...
for sentence in sentences:
    ...
    for i, tok in enumerate(tokens):
        ...
        shap_value = shap_dict.get(tok, 0.0)

        importance_by_position[bert_pos] += shap_value
        count_by_position[bert_pos] += 1


# Media por posición
mean_importance_by_position = np.zeros(8)

for i in range(8):
    if count_by_position[i] > 0:
        mean_importance_by_position[i] = importance_by_position[i] / count_by_position[i]
    else:
